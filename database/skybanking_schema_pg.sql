...
CREATE OR REPLACE FUNCTION update_last_modified_column() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP;
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- ==========================
...
UPDATE ON users FOR EACH ROW EXECUTE FUNCTION update_last_modified_column();
-- ==========================
-- Accounts table
-- ==========================
...
UPDATE ON accounts FOR EACH ROW EXECUTE FUNCTION update_last_modified_column();
-- ==========================
-- Transactions table
-- ==========================
CREATE TABLE transactions (
    ...
    FOREIGN KEY (receiver_account_id) REFERENCES accounts(account_id) ON DELETE
    SET NULL
);
...
UPDATE ON transactions FOR EACH ROW EXECUTE FUNCTION update_last_modified_column();
-- ==========================
...
CREATE TRIGGER update_admins_modtime BEFORE
UPDATE ON admins FOR EACH ROW EXECUTE FUNCTION update_last_modified_column();
-- ==========================
-- Admin Logs table
-- ==========================
...
CREATE TRIGGER update_settings_modtime BEFORE
UPDATE ON system_settings FOR EACH ROW EXECUTE FUNCTION update_last_modified_column();
-- ==========================
-- Notifications table
-- ==========================
...
IF p_txn_type IN ('WITHDRAWAL', 'TRANSFER')
AND v_current_balance < p_total_amount THEN RAISE EXCEPTION 'Insufficient balance';
END IF;
v_reference_number := 'TXN' || EXTRACT(
    EPOCH
    FROM NOW()
)::BIGINT || LPAD(p_account_id::TEXT, 6, '0');
...
WHERE account_id = p_account_id;
END IF;
IF p_txn_type = 'TRANSFER'
AND p_receiver_account_id IS NOT NULL THEN
UPDATE accounts
...
-- Triggers
-- ==========================
CREATE OR REPLACE FUNCTION log_user_update() RETURNS TRIGGER AS $$ BEGIN IF OLD.password_hash != NEW.password_hash THEN
...
VALUES (
        ...
        'PROFILE_UPDATE',
        'SYSTEM',
        'Account status changed to ' || CASE
...
AFTER
UPDATE ON users FOR EACH ROW EXECUTE FUNCTION log_user_update();
CREATE OR REPLACE FUNCTION log_transaction_create() RETURNS TRIGGER AS $$ BEGIN
...
AFTER
INSERT ON transactions FOR EACH ROW EXECUTE FUNCTION log_transaction_create();