        ...
        try {
            ...
            try (ResultSet rs = ps.getGeneratedKeys()) {
                if (rs.next()) {
                    ...
                    // Notify admins
                    try (PreparedStatement notif = con.prepareStatement(
                            "INSERT INTO notifications (title, message, type) VALUES (?, ?, ?)")) {
                        notif.setString(1, "New Loan Application");
                        notif.setString(2, "User ID " + userId + " applied for a loan of ₹" + principal + ".");
                        notif.setString(3, "INFO");
                        notif.executeUpdate();
                    }
                }
            }
    ...
    private void loadStatus(HttpServletRequest req) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                row.put("admin_reason", rs.getString("admin_reason"));
                loans.add(row);
            }
